...
{{ backup }}-script:
  file.managed:
    ...
    - context:
      ...
      remote_command: '/usr/bin/rdiff-backup --remote-schema "ssh -i /etc/backup/.ssh/id_rsa_{{ backup }} -C %s rdiff-backup --server" {{ config['source_directory'] }} {{ config['target_user'] }}@{{ config['target_host'] }}::{{ config['target_directory'] }}'
